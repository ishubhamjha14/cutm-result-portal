...
export const HomePage: React.FC<HomePageProps> = ({ onOpenGradesModal }) => {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      {/* Hero Section */}
      <section className="relative pt-10 pb-14 sm:pt-16 sm:pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center space-y-7">
        
        {/* University Pill Badge */}
        <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-600 dark:text-indigo-300 text-xs sm:text-sm font-medium backdrop-blur-md shadow-sm">
        ...
        <div className="space-y-3 max-w-3xl mx-auto">
          <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight text-slate-900 dark:text-white font-heading">
            CUTM RESULT PORTAL
          </h1>
        ...
        <div className="max-w-xl mx-auto mt-8">
          <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl relative overflow-hidden text-left bg-white/95 dark:bg-slate-900/90">
            
            <div className="flex items-center justify-between pb-5 border-b border-slate-200 dark:border-slate-800/80 mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-indigo-600/10 dark:bg-indigo-600/20 border border-indigo-500/20 dark:border-indigo-500/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
              ...
              <span className="px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 text-[11px] font-semibold flex items-center gap-1">
            ...
            <form onSubmit={handleSearch} className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-500 font-normal">e.g. 24CSE12345</span>
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-300 dark:border-slate-700/80 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-sm font-mono tracking-wider focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all uppercase"
                    autoFocus
                  />
                  {regNo && (
                    <button
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-300 dark:border-slate-700/80 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all cursor-pointer font-medium"
                >
                  {(metadata?.semesters || [
              ...
              {/* Error Message Alert */}
              {errorMessage && (
                <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-300 text-xs flex items-start gap-3 animate-in fade-in duration-200">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-sm tracking-wide shadow-xl shadow-indigo-500/25 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed group"
              >
                {isLoading ? (
            ...
            <div className="mt-6 pt-5 border-t border-slate-200 dark:border-slate-800/80 space-y-2">
              <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">
                <Sparkles className="w-3.5 h-3.5 text-indigo-500 dark:text-indigo-400" /> Quick Demo Test Accounts:
              </span>
              <div className="flex flex-wrap gap-2 pt-1">
                {[
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/90 hover:bg-slate-200 dark:hover:bg-slate-800 text-[11px] font-mono text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-300 dark:border-slate-800 hover:border-indigo-500/50 transition-all cursor-pointer flex items-center gap-1.5"
                  >
                    <span className="text-indigo-600 dark:text-indigo-400 font-bold">{demo.reg}</span>
                    <span className="text-slate-500 dark:text-slate-400">({demo.name}, Sem {demo.sem})</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-200 dark:border-slate-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 bg-white/90 dark:bg-slate-900/50">
        ...
        <div className="mt-12 p-6 rounded-2xl bg-gradient-to-r from-indigo-50 via-slate-100 to-purple-50 dark:from-indigo-950/60 dark:via-slate-900 dark:to-purple-950/60 border border-indigo-500/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">
                Understand how letter grades (O, E, A, B, C, D, F, M, S) translate into grade points.
              </p>
            </div>
          </div>
          <button
            onClick={onOpenGradesModal}
            className="px-5 py-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white border border-slate-300 dark:border-slate-700 text-xs font-semibold shadow-sm transition-all cursor-pointer shrink-0"
          >
