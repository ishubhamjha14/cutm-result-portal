  ...
  return (
    <>
      ...
      <div className="no-print space-y-4 sm:space-y-6 max-w-5xl mx-auto">
        ...
        <div
          ref={marksheetRef}
          className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-3.5 sm:p-6 md:p-10 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl relative overflow-hidden"
        >
          {/* Subtle Watermark Emblem Background (hidden on mobile to prevent overlapping) */}
          <div className="hidden sm:block absolute right-4 top-1/3 -translate-y-1/2 opacity-[0.02] pointer-events-none select-none text-slate-900 dark:text-slate-100">
            <GraduationCap className="w-64 sm:w-96 h-64 sm:h-96" />
          </div>

          {/* University Official Header */}
          <div className="text-center pb-4 sm:pb-6 border-b-2 border-indigo-500/20 space-y-1.5 sm:space-y-2">
            <div className="inline-flex items-center gap-1.5 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 text-[9.5px] sm:text-xs font-semibold uppercase tracking-wider text-center max-w-full leading-tight">
            ...
            <h1 className="text-lg sm:text-2xl md:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white font-heading uppercase leading-snug">
              Centurion University of Technology and Management
            </h1>
            <p className="text-[10px] sm:text-xs md:text-sm text-slate-600 dark:text-slate-400 font-medium max-w-xl mx-auto">
              {student.campus || 'Bhubaneswar Campus, Jatni, Odisha - 752050'}
            </p>
            
            <div className="pt-1">
              <span className="inline-block px-3 sm:px-4 py-1 sm:py-1.5 rounded-lg sm:rounded-xl bg-slate-100 dark:bg-slate-800 text-indigo-700 dark:text-indigo-300 font-bold text-[10px] sm:text-xs uppercase tracking-widest border border-slate-200 dark:border-slate-700 shadow-inner">
          ...
          <div className="mt-4 sm:mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-4 p-3 sm:p-5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 text-xs">
          ...
          {/* Summary Metric Callouts (SGPA, CGPA, Credits, Verdict) */}
          <div className="mt-4 sm:mt-6 grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
            
            <div className="p-3.5 sm:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-br from-indigo-50 to-indigo-100/50 dark:from-indigo-950/70 dark:to-slate-900 border-2 border-indigo-500/30 relative overflow-hidden shadow-sm dark:shadow-lg">
              <div className="flex items-center justify-between text-indigo-700 dark:text-indigo-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1 sm:mb-2">
                <span>Semester SGPA</span>
                <Award className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-indigo-600 dark:text-indigo-400" />
              </div>
              <div className="flex items-baseline gap-1.5 sm:gap-2">
                <span className="text-2xl sm:text-4xl font-extrabold text-slate-900 dark:text-white font-mono tracking-tight">
                  {sgpa.toFixed(2)}
                </span>
                <span className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 font-semibold">/ 10.00</span>
              </div>
              <p className="text-[9.5px] sm:text-[11px] text-indigo-600/90 dark:text-indigo-300/80 mt-1 sm:mt-2 truncate">
                {semester_name} Performance Index
              </p>
            </div>

            <div className="p-3.5 sm:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-br from-purple-50 to-purple-100/50 dark:from-purple-950/70 dark:to-slate-900 border-2 border-purple-500/30 relative overflow-hidden shadow-sm dark:shadow-lg">
              <div className="flex items-center justify-between text-purple-700 dark:text-purple-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1 sm:mb-2">
                <span>Cumulative CGPA</span>
                <TrendingUp className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-purple-600 dark:text-purple-400" />
              </div>
              <div className="flex items-baseline gap-1.5 sm:gap-2">
                <span className="text-2xl sm:text-4xl font-extrabold text-slate-900 dark:text-white font-mono tracking-tight">
                  {cgpa.toFixed(2)}
              ...
              <p className="text-[9.5px] sm:text-[11px] text-purple-600/90 dark:text-purple-300/80 mt-1 sm:mt-2 truncate">
                Overall academic score
              </p>
            </div>

            <div className="p-3.5 sm:p-5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-1.5">
              <span className="text-slate-600 dark:text-slate-400 text-[10px] sm:text-xs font-semibold uppercase tracking-wider block">
                Credit Metrics
              </span>
            ...
            <div className="p-3.5 sm:p-5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
              <span className="text-slate-600 dark:text-slate-400 text-[10px] sm:text-xs font-semibold uppercase tracking-wider block">
                Result Verdict
              </span>
              <div className="my-auto py-1">
                <span
                  className={`inline-flex items-center gap-1.5 px-3 sm:px-4 py-1 sm:py-1.5 rounded-xl text-xs sm:text-sm font-extrabold uppercase tracking-wider border ${
                    isPassed
                      ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30'
                      : 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/30'
                  }`}
                >
                  {isPassed ? <CheckCircle className="w-3.5 sm:w-4 h-3.5 sm:h-4" /> : <AlertCircle className="w-3.5 sm:w-4 h-3.5 sm:h-4" />}
                  <span>{result_status}</span>
                </span>
              </div>
              <span className="text-[9.5px] sm:text-[10px] text-slate-500 pt-0.5 truncate">
          ...
          <div className="mt-5 sm:mt-8 space-y-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center font-mono font-semibold text-slate-800 dark:text-slate-200">
                          {['R', 'M', 'S'].includes(sub.grade) ? '—' : sub.grade_point.toFixed(1)}
                        </td>
            ...
            {/* 2. Dedicated Responsive Mobile Presentation (< 768px) */}
            <div className="block md:hidden space-y-2.5">
              {subjects.map((sub, index) => {
                ...
                return (
                  <div
                    key={sub.subject_code}
                    className="p-3.5 rounded-2xl bg-white dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 space-y-2.5 shadow-sm transition-all"
                  >
                    {/* Index & Subject Code */}
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono font-extrabold text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/70 px-2 py-0.5 rounded-md border border-indigo-500/20 tracking-wider">
                        {sub.subject_code}
                      </span>
                    </div>

                    {/* Course Title */}
                    <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase leading-snug tracking-tight">
                      {sub.subject_name}
                    </h4>

                    {/* Credits | Grade | GP Grid */}
                    <div className="grid grid-cols-3 gap-2 py-2 border-y border-slate-100 dark:border-slate-800/80 text-center">
                      <div>
                        <span className="text-[9.5px] uppercase tracking-wider text-slate-400 dark:text-slate-500 block font-semibold mb-0.5">
                          Credits
                        </span>
                      ...
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded-md font-mono font-extrabold text-xs border ${getScreenGradeBadge(sub.grade)}`}>
                          {sub.grade}
                        </span>
                      </div>
                    ...
                    {/* Credit Points on Left, PASS/FAIL Status on Right */}
                    <div className="flex items-center justify-between pt-0.5">
                      <div>
                        <span className="text-[9px] uppercase tracking-wider text-slate-400 dark:text-slate-500 block font-semibold">
                          Credit Points
                        </span>
          ...
          {/* ========================================================================= */}
          {/* MOBILE PAGE 2 — ACADEMIC PERFORMANCE ANALYTICS SECTION                     */}
          {/* ========================================================================= */}
          <div className="no-print mt-8 pt-6 sm:mt-10 sm:pt-8 border-t-2 border-dashed border-slate-200 dark:border-slate-800 space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 font-heading">
                    Academic Performance Analytics
                  </h3>
                  <p className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400">
                    Semester-wise progression trajectory & cumulative growth
                  </p>
                </div>
              </div>
            ...
            {chartData.length > 1 ? (
              <div className="p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800/80 space-y-3">
              ...
              <div className="p-4 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800/80 text-xs text-slate-500 text-center py-6">
                Single semester record registered. Progression chart activates upon subsequent semester results.
              </div>
            )}
          </div>

          {/* Screen Verification / Disclaimer */}
          <div className="mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-slate-200 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 items-center text-xs text-slate-500 dark:text-slate-400">
